%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%% Generates first set of template features%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%Various features are obtained by taking the correlation, covariance and L2 
%norm between the templates and the individual ERP signals. 
...
test=data(:,1);
        ...
        for l=2:58
            ...
            for sub=training_set
                for j=1:5
                    sesh=j;
                    disp(sprintf('train data, chan=%s, subject=%d, session=%d', chan,sub,j))
            ...
            %get correlations and covariances with postive feedback and negative feedback templates
            row_count=0;
            pf_cor_collect=zeros(4080,1);
            nf_cor_collect=zeros(4080,1);
            pf_cov_collect=zeros(4080,1);
            nf_cov_collect=zeros(4080,1);
            pf_l2_collect=zeros(4080,1);
            nf_l2_collect=zeros(4080,1);
            for sub=training_set
                for j=1:5
                    sesh=j;
                    %sprintf('test data, offset=%d, chan=%s, subject=%d, session=%d', offset_vec(jj), chan,sub,j)
                    ...
                    for i=1:length(ind)
                        row_count=row_count+1;
                        pf_cor=corr(data_filt(ind(i)+lb:ind(i)+rb),pf_sig);
                        pf_cor_collect(row_count)=pf_cor;
                        nf_cor=corr(data_filt(ind(i)+lb:ind(i)+rb),nf_sig);
                        nf_cor_collect(row_count)=nf_cor;
                        pf_cov=cov(data_filt(ind(i)+lb:ind(i)+rb),pf_sig);
                        pf_cov=pf_cov(1,2);
                        pf_cov_collect(row_count)=pf_cov;
                        nf_cov=cov(data_filt(ind(i)+lb:ind(i)+rb),nf_sig);
                        nf_cov=nf_cov(1,2);
                        nf_cov_collect(row_count)=nf_cov;
                        pf_l2_collect(row_count)=norm(data_filt(ind(i)+lb:ind(i)+rb)-pf_sig);
                        nf_l2_collect(row_count)=norm(data_filt(ind(i)+lb:ind(i)+rb)-nf_sig);
            ...
            eval(sprintf('train.%s_pf_cor=pf_cor_collect;',chan))
            eval(sprintf('train.%s_nf_cor=nf_cor_collect;',chan))
            eval(sprintf('train.%s_cor_diff=pf_cor_collect-nf_cor_collect;',chan))
            eval(sprintf('train.%s_pf_cov=pf_cov_collect;',chan))
            eval(sprintf('train.%s_nf_cov=nf_cov_collect;',chan))
            eval(sprintf('train.%s_cov_ratio=pf_cov_collect./nf_cov_collect;',chan))
            eval(sprintf('train.%s_pf_l2=pf_l2_collect;',chan))
            eval(sprintf('train.%s_nf_l2=nf_l2_collect;',chan))
            eval(sprintf('train.%s_nf_l2_diff=nf_l2_collect-pf_l2_collect;',chan))
            ...
            eval(sprintf('test.%s_pf_cor=pf_cor_collect;',chan))
            eval(sprintf('test.%s_nf_cor=nf_cor_collect;',chan))
            eval(sprintf('test.%s_cor_diff=pf_cor_collect-nf_cor_collect;',chan))
            eval(sprintf('test.%s_pf_cov=pf_cov_collect;',chan))
            eval(sprintf('test.%s_nf_cov=nf_cov_collect;',chan))
            eval(sprintf('test.%s_cov_ratio=pf_cov_collect./nf_cov_collect;',chan))
            eval(sprintf('test.%s_pf_l2=pf_l2_collect;',chan))
            eval(sprintf('test.%s_nf_l2=nf_l2_collect;',chan))
            eval(sprintf('test.%s_nf_l2_diff=nf_l2_collect-pf_l2_collect;',chan))
        ...
        %save files 
        writetable(train,strcat(dir_info.features,'training_set_template_features1.csv')) %save files
        writetable(test,strcat(dir_info.features,'test_set_template_features1.csv')) %save files
